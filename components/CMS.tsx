...
const CMS: React.FC = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    const data = await mockService.getColleges();
    setColleges(data);
    setLoading(false);
  };
  ...
  const loadStudents = () => {
    if (selectedCollege) {
      const data = mockService.getDetailedStudentAudit(selectedCollege.id);
      const enhancedData = data.map((s, idx) => ({
        ...s,
        workshopName: idx % 2 === 0 ? 'React & AI Masterclass' : 'Advanced Architecture',
  ...
  const handleManualSubmit = async (e: React.FormEvent) => {
    ...
    await mockService.addStudents([newStudent]);
    loadStudents();
  ...
  const handleImportCSV = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      if (newStudents.length > 0) { 
        await mockService.addStudents(newStudents); 
        loadStudents(); 
      }
      setImportFeedback({ success: newStudents.length, errors: errorsList }); 
      setIsImporting(false);
    };
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500 no-print">
      <div className="flex flex-col lg:flex-row justify-between items-center gap-6">
        ...
        <div className="flex gap-4 w-full lg:w-auto">
          <button onClick={() => fileInputRef.current?.click()} disabled={isImporting} className="flex-1 lg:flex-none flex items-center justify-center gap-2 px-6 py-4 bg-white border border-slate-200 text-[10px] font-black uppercase tracking-widest text-slate-600 hover:bg-slate-50 transition-all rounded-[20px] shadow-sm">{isImporting ? <Loader2 size={16} className="animate-spin" /> : <Upload size={16} />} Import Students CSV</button>
          <input type="file" className="hidden" accept=".csv" ref={fileInputRef} onChange={handleImportCSV} />
          <button onClick={() => setIsManualModalOpen(true)} className="flex-1 lg:flex-none flex items-center justify-center gap-2 px-8 py-4 bg-indigo-600 text-white text-[10px] font-black uppercase tracking-widest hover:bg-indigo-700 transition-all rounded-[20px] shadow-lg"><UserPlus size={16} /> Add Student Manually</button>
        </div>
      </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredStudents.length === 0 ? (
                <tr><td colSpan={4} className="p-20 text-center"><div className="flex flex-col items-center justify-center text-slate-300"><Users size={64} className="mb-4 opacity-10" /><p className="font-black uppercase tracking-[4px] text-sm">No Student Data</p></div></td></tr>
              ) : (
                filteredStudents.map((student) => (
                  <tr key={student.id} className="hover:bg-slate-50/80 transition-all group">
                    ...
                    <td className="px-10 py-6 text-right"><button onClick={() => setSelectedStudent(student)} className="p-3 text-slate-300 hover:text-indigo-600 hover:bg-indigo-50 rounded-2xl transition-all shadow-sm bg-white border border-slate-100"><Eye size={18} /></button></td>
                  </tr>
                ))
              )}
            </tbody>
